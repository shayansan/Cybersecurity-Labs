...
# it has the same name (ping) on CMD.

2- traceroute => is used to discover the path that packets take from your system to a destination host.
...
# its name is tracert on CMD.

3- nslookup=> is used to query DNS servers and retrieve DNS records for a domain name or an IP address.
...
# it has the same name (nslookup) on CMD.

4- dig=> is a DNS query tool used to manually ask DNS servers for detailed information about domain names.
...
# it's the same in window but you have to install a packet before it(ISC.BIND).
